...
function read_header(ctx::SHMContext)
    ...
    tda_h1                = reinterpret(Float64, raw_hdr[33:40])[1]
    tda_h2                = reinterpret(Float64, raw_hdr[41:48])[1]
    re_lambda_max         = reinterpret(Float64, raw_hdr[49:56])[1]

    return SHMHeader(
        write_index,
        last_updated_epoch_ns,
        active_node_count,
        state_flags,
        mean_ricci,
        tda_h1,

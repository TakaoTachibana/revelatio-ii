...
// HTTP JSON のバインド設定（snake_case も camelCase もケースインセンシティブで柔軟受取）
...
app.MapPost("/api/v1/topology/event", async (TopologyEventRequest request, CytoplasmV4Reader reader, IConfiguration config) => {
		Console.WriteLine($"[Gateway Recived] Ricce: {request.MeanRicciCurvature}, ReLambda: {request.ReLambdaMax}, H1: {request.TdaH1Persistence}");

		var triggerPosts = reader.ReadTopTriggerPosts();
		Console.WriteLine($"[Gateway Trigger Check] Trigger Count: {triggerPosts.Count}");

		if (triggerPosts.Count == 0) {
				triggerPosts.Add(new TriggerPostDto(
						1,
						"at://did:plc:test/app.bsky.feed.post/123",
						"did:plc:revelatio_demo",
						"【幾何学的特異点検知】言論空間の局所曲率が臨界面に達しました。",
						0.9852,
						-0.8921,
    ...
    // 引数に jsonOptions を渡して camelCase 化を強制
...
// DTO: 属性を外し C# / Minimal API の柔軟ケースバインドに任せる
